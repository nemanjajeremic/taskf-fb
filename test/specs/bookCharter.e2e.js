...
describe("My Login application", () => {
  it("Verify that user can access specific charter", () => {
    browser.url(
      `https://${loginCredentials.username}:${loginCredentials.pwd}@qahiring.dev.fishingbooker.com/charters/view/19612`
    );

    expect(CharterPage.charterTitleLabel).toHaveText(
      "Fisherman's Friend Charter"
    );
  });

  it("Verify that user can enter number of days, adults and children", () => {
    CharterPage.enterCharterCriteria(
      charterData.charter.days,
      charterData.charter.adults,
      charterData.charter.children
    );

    expect(CharterPage.amountOfSelectedDays).toEqual(charterData.charter.days);
    expect(CharterPage.amountOfSelectedChidren).toEqual(
      charterData.charter.children
    );
    expect(CharterPage.amountOfSelectedAdults()).toEqual(
      charterData.charter.adults
    );
  });

  it("Verify that user can book charter and access user details page", () => {
    CharterPage.findAndBookFirstAvailableDate(charterData.charter.tripType[0]);
  ...
  it("Verify that user can enter user data and proceed to payment", () => {
    ...
    DetailsPage.enterUserDetails(
      charterData.user.firstName,
      charterData.user.lastName,
      charterData.user.phoneNumber,
      charterData.user.message
    );
  ...
  it("Verify that user can enter payment data and proced to confirmation page", () => {
    ...
    expect(PaymentPage.finalizingPaymentMessage).toHaveText(
